...
<article class="course-card{{ $cardUrl ? ' course-card--clickable' : '' }}" itemscope itemtype="https://schema.org/Course">
    ...
    @if (!empty($course['is_enrolled']))
        <div class="course-card__progress">
            ...
            <div class="course-card__progress-meta">{{ $course['progress'] ?? 0 }}% concluido</div>
        </div>
    @endif
    ...
    @if ($showButtons)
        <div class="course-card__actions">
            @if (!empty($course['is_enrolled']))
                <a class="btn btn-primary" href="{{ $course['access_url'] ?? '#' }}">Acessar</a>
                <span class="btn btn-ghost" aria-disabled="true">Matriculado</span>
            @else
